...
class PatientController extends Controller
{
    public function __invoke($s, $w, $c){
        if (($s != 0) && ($w != 0)){
            // if ($c==0){
            //     return User::select('nom', 'prenom', 'phone', 'wilaya', 'commune', 'adresse', 'cabinet')->where('specialite', $s)->where('wilaya', $w)->get(); 
            // }else{
            //     return User::select('nom', 'prenom', 'phone', 'wilaya', 'commune', 'adresse', 'cabinet')->where('specialite', $s)->where('wilaya', $w)->where('commune', $c)->get(); 
